...
public class AdvancedTmxMapLoader extends TmxMapLoader {

    private MapPacket mapWrapper;
     ...
     *
     *  */
    public AdvancedTmxMapLoader (MapPacket mapWrapper) {
...
//            root = xml.parse(tmxFile); // TODO: HERE
            root = xml.parse(mapWrapper.map);
            ObjectMap<String, Texture> textures = new ObjectMap<String, Texture>();
            Array<FileHandle> textureFiles = loadTilesets(root, tmxFile);
            textureFiles.addAll(loadImages(root, tmxFile));

            for (FileHandle textureFile : textureFiles) {
                // Find the texture in the MapWRapper
                int index = Arrays.asList(mapWrapper.tilesets).indexOf(textureFile.name());
//                Pixmap pixmap = new Pixmap(mapWrapper.images[index], 0, mapWrapper.images[index].length);
//                Gdx2DPixmap pixmap = new Gdx2DPixmap(mapWrapper.images[index], 0, mapWrapper.images[index].length, Gdx2DPixmap.GDX2D_FORMAT_RGB888);
                Pixmap pixmap = new Pixmap(mapWrapper.pixels[index].length, mapWrapper.pixels[index][0].length, Pixmap.Format.RGBA8888);
                for (int x = 0; x < pixmap.getWidth(); x ++) {
                    for (int y = 0; y < pixmap.getHeight(); y ++) {
                        pixmap.drawPixel(x, y, mapWrapper.pixels[index][x][y]);
                    }
                }

                Texture texture = new Texture(pixmap, parameters.generateMipMaps);
...
//                tileset = xml.parse(tsxFile); // TODO HERE
                tileset = xml.parse(mapWrapper.map);
...
//                try {
//                    element = xml.parse(tsx); // TODO HERE
                    element = xml.parse(mapWrapper.map);
